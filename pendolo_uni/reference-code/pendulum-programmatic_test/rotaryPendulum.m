%% Quanser Rotary Inverted Pendulum model
...
laggingAnalysis = true; % to plot continuous and discrete responses to impulse
...
x0 = [0; 0.174; 0; 0.767];
%%
% Creates a continuous-time state-space model with sample time Ts using |ss|
sysc = ss(cA,cB,cC,[]);

%% 
% Convert continuous-time dynamic system to discrete time
sys = c2d(sysc,Ts);
